ple0.jack.fn <- function(y, x, c, h, kernel="epanechnikov"){
  ...
  return(c(tau.ple0=tau.ple0, se.ple0=hl.ple0))
  
}

ple1.jack.fn <- function(y.vec, x.vec, c, h, kernel="epanechnikov"){
  ...
  #first getting the original estimate without anything deleted
  n <- length(x.vec)
  l.ll.matrix <- sapply(1:n, LL.fn, x=x.vec, h=h, kernelname=mykernel)
  f.vec <- as.numeric(x.vec>c)
